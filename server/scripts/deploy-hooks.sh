...
echo "==> Copying hooks to /tmp..."
$SCP "$DIR/pb_hooks/notify.pb.js" "$SERVER:/tmp/notify.pb.js"
$SCP "$DIR/pb_hooks/join.pb.js" "$SERVER:/tmp/join.pb.js"

echo "==> Installing into $REMOTE_HOOKS (sudo)..."
$SSH $SERVER "sudo mkdir -p $REMOTE_HOOKS && sudo mv /tmp/notify.pb.js /tmp/join.pb.js $REMOTE_HOOKS/ && sudo chown -R pocketbase:pocketbase $REMOTE_HOOKS"

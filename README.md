...
- **Where state lives.** Everything is server-truth via Riverpod controllers
  ...
  `catalog_snapshot_v1` - the last successful catalog fetch, served
  snapshot-first on every start), the cached_network_image disk cache
  (remote art bytes),
...
- **The art system is two layers.** Bundled: three id→asset registries -
  ...
  **Widgets must read art via `ref.watch(catalogProvider).lookupX(id)` and
  ...
  read `selectableCatalogProvider` instead. Snapshot-first: the last
  successful fetch (SharedPreferences, warmed from `AppRoot` during splash)
  is served immediately so chosen remote art resolves on the first frame,
  then the live fetch refreshes it in the background; fail-soft chain:
  snapshot → live fetch → bundled-only (first run only). New art ships
...
- **NFC flows open the app via the OS, not in-app reading.** A tag holds
  ...
  opens the subject page per the You tab's per-device toggle. The app never
